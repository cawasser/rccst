...
(def sample-data-2
  (r/atom
    {:nodes [
             {:id       ":fn/range",                        ;:type ":source/fn",
              :data {:label   ":fn/range",
                     :inputs {":topic/coverage-data" [":data" ":data"]},
                     :outputs {":topic/time-range" [":range" ":data"]}},
              :position {:x 0, :y 0}}
             {:id                            ":ui/globe",   ;:type ":ui/component",
              :data {:label   ":ui/globe",
                     :inputs {":topic/shapes"       [":data" ":shapes"],
                              ":topic/current-time" [":data" ":current-time"]},
                     :outputs {}}, :position {:x 0, :y 86}}
             {:id       ":topic/coverage-data",             ;:type ":source/remote",
              :data {:label   ":topic/coverage-data",
                     :inputs {},
                     :outputs {":fn/coverage" [":data" ":coverages"],
                               ":fn/range"    [":data" ":data"]}},
              :position {:x 0, :y 172}}
             {:id       ":topic/shapes",                    ;:type ":source/local",
              :data {:label   ":topic/shapes",
                     :inputs {":fn/coverage" [":shapes" ":data"]},
                     :outputs {":ui/globe" [":data" ":shapes"]}},
              :position {:x 0, :y 258}}
             {:id       ":ui/satellites",                   ;:type ":ui/component",
              :data {:label   ":ui/satellites",
                     :inputs {":topic/satellite-data" [":data" ":data"]},
                     :outputs {":topic/satellite-data"      [":data" ":data"],
                               ":topic/selected-satellites" [":selection" ":data"]}},
              :position {:x 0, :y 344}}
             {:id       ":topic/current-time",              ;:type ":source/local",
              :data {:label   ":topic/current-time",
                     :inputs {":ui/time-slider" [":value" ":data"]},
                     :outputs {":ui/current-time" [":data" ":value"],
                               ":ui/time-slider"  [":data" ":value"],
     ...
     :edges [
             {:targetHandle  ":data", :animated false,
              ...
              :arrowHeadType "arrowclosed", :label ":data", :id "0",
             ...
             {:targetHandle  ":value", :animated false,
              ...
              :arrowHeadType "arrowclosed", :label ":value", :id "6",
              :sourceHandle  ":data", :target ":ui/current-time"}
             {:targetHandle  ":value", :animated false,
              :source       ":topic/current-time", :style {:strokeWidth 1, :stroke :black},
              :arrowHeadType "arrowclosed", :label ":value", :id "7",
              :sourceHandle  ":data", :target ":ui/time-slider"}
             {:targetHandle  ":current-time", :animated false,
              :source       ":topic/current-time", :style {:strokeWidth 1, :stroke :black},
              :arrowHeadType "arrowclosed", :label ":current-time", :id "8",
              :sourceHandle  ":data", :target ":ui/globe"}
             {:targetHandle  ":current-time", :animated false,
              :source       ":topic/current-time", :style {:strokeWidth 1, :stroke :black},
              :arrowHeadType "arrowclosed", :label ":current-time", :id "9",
              :sourceHandle  ":data", :target ":fn/coverage"}
             {:targetHandle  ":data", :animated false,
              :source       ":fn/coverage", :style {:strokeWidth 1, :stroke :black},
              :arrowHeadType "arrowclosed", :label ":data", :id "10",
...
(defn- flow* [& {:keys [component-id nodes edges
  ...
  [:> ReactFlow {:nodes               nodes
                 ...
                 ;:nodeTypes           {":ui/component" (partial custom-node :ui/component)
                 ;                      ":source/remote" (partial custom-node :source/remote)
                 ;                      ":source/local" (partial custom-node :source/local)
                 ;                      ":source/fn" (partial custom-node :source/fn)}
                 ;:edgeTypes           {}
